...
def init_layer(layer):
    ...
    if hasattr(layer, 'bias'):
        if layer.bias is not None:          #layer.bias不是空
...
class ConvBlock(nn.Module):                 #卷积块
    def __init__(self, in_channels, out_channels):          #通道
        
        super(ConvBlock, self).__init__()                   #super()调用父类（超类）的方法
    ...
    def forward(self, input, pool_size=(2, 2), pool_type='avg'):
        ...
        x = input
        x = F.relu_(self.bn1(self.conv1(x)))            #relu函数
        x = F.relu_(self.bn2(self.conv2(x)))
        if pool_type == 'max':
            x = F.max_pool2d(x, kernel_size=pool_size)      #kernel_size表示做最大池化的窗口大小
        elif pool_type == 'avg':
...
class MobileNetV2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.spectrogram_extractor = Spectrogram(n_fft=window_size, hop_length=hop_size, 
            win_length=window_size, window=window, center=center, pad_mode=pad_mode, 
            freeze_parameters=True)         #短时傅里叶变换并得到信号的频谱图

        # Logmel feature extractor
        self.logmel_extractor = LogmelFilterBank(sr=sample_rate, n_fft=window_size, 
            n_mels=mel_bins, fmin=fmin, fmax=fmax, ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)       #logmel频谱图

        # Spec augmenter
        self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2, 
            freq_drop_width=8, freq_stripes_num=2)      #SpecAugment是一种log梅尔声谱层面上的数据增强方法
...
class Wavegram_Logmel_Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        # Spectrogram extractor 光谱图提取器
        self.spectrogram_extractor = Spectrogram(n_fft=window_size, hop_length=hop_size, 
            win_length=window_size, window=window, center=center, pad_mode=pad_mode, 
            freeze_parameters=True)

        # Logmel feature extractor  Logmel特征提取器
        self.logmel_extractor = LogmelFilterBank(sr=sample_rate, n_fft=window_size, 
            n_mels=mel_bins, fmin=fmin, fmax=fmax, ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)

        # Spec augmenter    规格扩充器
        self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2, 
            freq_drop_width=8, freq_stripes_num=2)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        # Concatenate Wavegram and Log mel spectrogram along the channel dimension  沿通道维度连接波形图和对数梅尔谱图
...
class Wavegram_Logmel_Cnn14_mcb(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num):
...
class Wavegram_Logmel_Cnn14_attention(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = x.repeat(1, 2, 1, 1)
        a1 = a1.repeat(1, 2, 1, 1)

        x = nn.Linear(x,1,bias=False)

        alpha_x = F.softmax(F.leaky_relu(x))
        alpha_a1 = F.softmax(F.leaky_relu(a1))



        x = alpha_x*x +  alpha_a1*a1
